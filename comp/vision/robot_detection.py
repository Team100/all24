# pylint: disable=C0103,C0114,C0115,C0116,R0902,R0914,R0915,W0201,W0612
...
class GamePieceFinder:
    def __init__(self, serial: str, camList: list[CameraData]) -> None:
        ...
        self.object_lower = np.array((0, 0, 0))
        self.object_higher = np.array((180, 25, 150))
        self.frame_time = 0
        self.theta = 0
    ...
    def find_object(self, img_yuv: Mat, camera: CameraData):
        ...
        floodfill: MatLike = img_range.copy()
        h, w = img_range.shape[:2]
        mask: MatLike = np.zeros((h + 2, w + 2), np.uint8)
