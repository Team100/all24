...
public class DriveToAmp extends SequentialCommandGroup {
    public DriveToAmp(
        ...
        if (alliance == Alliance.Blue) {
            addCommands(new DriveWithProfile2(() -> new Pose2d(1.834296, 6.474794, new Rotation2d(Math.PI / 2)), drive,
                    new HolonomicDriveController100(), limits),
                    new ParallelCommandGroup(new AmpSet(amp, 1.8),
                    new SequentialCommandGroup(new DriveWithProfile2(() -> new Pose2d(1.834296, 6.799454, new Rotation2d(Math.PI / 2)), drive,
                            new HolonomicDriveController100(), limits),
                            new ParallelCommandGroup(ampFeeder.run(ampFeeder::outtake), new WaitCommand(1)))));
        } else {
            addCommands(
                    new ParallelDeadlineGroup(new DriveWithProfile2(() -> new Pose2d(1.834296, 8.221 - 6.474794, new Rotation2d(Math.PI / 2)),
                            drive,
                            new HolonomicDriveController100(), limits),
                            new FeedToAmp(intake, shooter, ampFeeder, feeder)),
                            new ParallelCommandGroup(new AmpSet(amp, 1.8), 
                    new SequentialCommandGroup(new DriveWithProfile2(() -> new Pose2d(1.834296, 8.221 - 6.799454, new Rotation2d(Math.PI / 2)),
                            drive,
                            new HolonomicDriveController100(), limits),
                            new ParallelCommandGroup(ampFeeder.run(ampFeeder::outtake), new WaitCommand(1))))
            );
