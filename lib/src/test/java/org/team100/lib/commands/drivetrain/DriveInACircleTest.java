...
class DriveInACircleTest {
    ...
    void testInitial() {
        ...
        Pose2d m_center = DriveInACircle.getCenter(initial, radiusM);
        assertEquals(0, m_center.getX(), kDelta);
        assertEquals(1, m_center.getY(), kDelta);
        assertEquals(0, m_center.getRotation().getRadians(), kDelta);

        // just starting, zero velocity, max acceleration
        double m_angle = 0;
        double m_speed = 0;
        double accel = 1;
        State100 m_rotation = new State100(0, 0, 0);

        SwerveState reference = DriveInACircle.getReference(m_center, radiusM, m_angle, m_speed, accel, m_rotation);
