package org.team100.lib.sensors.navx;
...
/**
 * Copy of AHRS.java, to work around the removal of RuntimeDetector.
 * 
 * https://github.com/wpilibsuite/allwpilib/issues/6823
 * 
 * This also implements the fix for SPI hanging described here:
 * 
 * https://www.chiefdelphi.com/t/navx2-disconnecting-reconnecting-intermittently-not-browning-out/425487
 * 
...
public class AHRS100 implements Sendable {

    /**
     * Identifies one of the three sensing axes on the navX sensor board. Note that
     * these axes are
     * board-relative ("Board Frame"), and are not necessarily the same as the
     * logical axes of the
     ...
     */
    public AHRS100(SPI.Port spi_port_id, byte update_rate_hz) {
        commonInit(update_rate_hz);
        if (m_simDevice != null) {
            io = new SimIO(update_rate_hz, io_complete_sink, m_simDevice);
        } else {
            // if (RuntimeDetector.isLinux() && !RuntimeDetector.isAthena() && spi_port_id
            // == SPI.Port.kMXP) {
    ...
    public AHRS100(SPI.Port spi_port_id, int spi_bitrate, byte update_rate_hz) {
        ...
        if (m_simDevice != null) {
            ...
            // io = new RegisterIOMau(update_rate_hz, io_complete_sink, board_capabilities);
            // } else {
            io = new RegisterIO(new RegisterIO_SPI(new SPI(spi_port_id), spi_bitrate), update_rate_hz, io_complete_sink,
                    board_capabilities);
            // }
        }
        SendableRegistry.addLW(this, "navX-Sensor", spi_port_id.value);
     ...
     * @param update_rate_hz Custom Update Rate (Hz)
     */
    public AHRS100(I2C.Port i2c_port_id, byte update_rate_hz) {
        Tracer.Trace("Instantiating navX-Sensor on I2C Port %s.\n", i2c_port_id.toString());
        commonInit(update_rate_hz);
        if (m_simDevice != null) {
            io = new SimIO(update_rate_hz, io_complete_sink, m_simDevice);
        } else {
            // if (RuntimeDetector.isLinux() && !RuntimeDetector.isAthena() && i2c_port_id
            // == I2C.Port.kMXP) {
            // io = new RegisterIOMau(update_rate_hz, io_complete_sink, board_capabilities);
            // } else {
     ...
     * @param update_rate_hz Custom Update Rate (Hz)
     */
    public AHRS100(SerialPort.Port serial_port_id, SerialDataType data_type, byte update_rate_hz) {
        Tracer.Trace("Instantiating navX-Sensor on Serial Port %s.\n", serial_port_id.toString());
        commonInit(update_rate_hz);
        if (m_simDevice != null) {
            io = new SimIO(update_rate_hz, io_complete_sink, m_simDevice);
        } else {
            // if (RuntimeDetector.isLinux() && !RuntimeDetector.isAthena() &&
            // serial_port_id == SerialPort.Port.kMXP) {
            // io = new RegisterIOMau(NAVX_DEFAULT_UPDATE_RATE_HZ, io_complete_sink,
            // board_capabilities);
            // }
            // else {
     ...
     * @param i2c_port_id I2C port to use
     */
    public AHRS100(I2C.Port i2c_port_id) {
     ...
     * @param serial_port_id SerialPort to use
     */
    public AHRS100(SerialPort.Port serial_port_id) {
    ...
    public void zeroYaw() {
        ...
        if (isCalibrating()) {
            double delta_time_since_last_yawreset_while_calibrating_request = curr_timestamp
                    - last_yawreset_while_calibrating_request_timestamp;
    ...
    public boolean isCalibrating() {
        return !((cal_status &
                AHRSProtocol.NAVX_CAL_STATUS_IMU_CAL_STATE_MASK) == AHRSProtocol.NAVX_CAL_STATUS_IMU_CAL_COMPLETE);
    }
     ...
     * be invoked.
     * This function transforms acceleration in G to meters/sec^2, then converts
     * this value to
     * Velocity in meters/sec (based upon velocity in the previous sample). Finally,
     * this value
     ...
     * 
     * @param callback         The callback object to be invoked when callbacks
     *                         occur
     * @param callback_context The callback context object to be passed as a
     *                         parameter
     ...
     * value
     * has not had acceleration due to gravity removed from it, and has not been
     * rotated to
     * the world reference frame. Gravity-corrected, world reference frame-corrected
     * X axis acceleration data is accessible via the
     * {@link #getWorldLinearAccelX()} method.
     * <p>
     * 
     ...
     * rotated to
     * the world reference frame. Gravity-corrected, world reference frame-corrected
     * Y axis acceleration data is accessible via the
     * {@link #getWorldLinearAccelY()} method.
     * <p>
     * 
     * @return Returns the current acceleration rate (in G).
     */
     ...
     * rotated to
     * the world reference frame. Gravity-corrected, world reference frame-corrected
     * Z axis acceleration data is accessible via the
     * {@link #getWorldLinearAccelZ()} method.
     * <p>
     * 
     * @return Returns the current acceleration rate (in G).
     */
     ...
     * uTesla). NOTE:
     * this value is unprocessed, and should only be accessed by advanced users.
     * This raw value
     * has not been tilt-corrected, and has not been combined with the other
     * magnetometer axis
     * data to yield a compass heading. Tilt-corrected compass heading data is
     * accessible
     ...
     * Software-
     * based yaw resets do not update the yaw angle component of the
     * sensor-generated
     * Quaternion values or the Fused Heading values.
     * 
     * @param enable true to enable board-level yaw resets; false to enable
     *               software-based
     *               yaw resets.
     */
    ...
    class IOCompleteNotification implements IIOCompleteNotification {
        ...
        public void setYawPitchRoll(YPRUpdate ypr_update, long sensor_timestamp) {
            AHRS100.this.yaw = ypr_update.yaw;
            AHRS100.this.pitch = ypr_update.pitch;
            AHRS100.this.roll = ypr_update.roll;
            AHRS100.this.compass_heading = ypr_update.compass_heading;
            AHRS100.this.last_sensor_timestamp = sensor_timestamp;
        }
        ...
        public void setAHRSPosData(AHRSPosUpdate ahrs_update, long sensor_timestamp) {
            ...
            AHRS100.this.yaw = ahrs_update.yaw;
            AHRS100.this.pitch = ahrs_update.pitch;
            AHRS100.this.roll = ahrs_update.roll;
            AHRS100.this.compass_heading = ahrs_update.compass_heading;
            yaw_offset_tracker.updateHistory(ahrs_update.yaw);

            /* Update AHRS class variables */

            // 9-axis data
            AHRS100.this.fused_heading = ahrs_update.fused_heading;

            // Gravity-corrected linear acceleration (world-frame)
            AHRS100.this.world_linear_accel_x = ahrs_update.linear_accel_x;
            AHRS100.this.world_linear_accel_y = ahrs_update.linear_accel_y;
            AHRS100.this.world_linear_accel_z = ahrs_update.linear_accel_z;

            // Gyro/Accelerometer Die Temperature
            AHRS100.this.mpu_temp_c = ahrs_update.mpu_temp;

            // Barometric Pressure/Altitude
            AHRS100.this.altitude = ahrs_update.altitude;
            AHRS100.this.baro_pressure = ahrs_update.barometric_pressure;

            // Status/Motion Detection
            AHRS100.this.is_moving = (((ahrs_update.sensor_status &
                    AHRSProtocol.NAVX_SENSOR_STATUS_MOVING) != 0)
                            ? true
                            : false);
            AHRS100.this.is_rotating = (((ahrs_update.sensor_status &
                    AHRSProtocol.NAVX_SENSOR_STATUS_YAW_STABLE) != 0)
                            ? false
                            : true);
            AHRS100.this.altitude_valid = (((ahrs_update.sensor_status &
                    AHRSProtocol.NAVX_SENSOR_STATUS_ALTITUDE_VALID) != 0)
                            ? true
                            : false);
            AHRS100.this.is_magnetometer_calibrated = (((ahrs_update.cal_status &
                    AHRSProtocol.NAVX_CAL_STATUS_MAG_CAL_COMPLETE) != 0)
                            ? true
                            : false);
            AHRS100.this.magnetic_disturbance = (((ahrs_update.sensor_status &
                    AHRSProtocol.NAVX_SENSOR_STATUS_MAG_DISTURBANCE) != 0)
                            ? true
                            : false);

            AHRS100.this.quaternionW = ahrs_update.quat_w;
            AHRS100.this.quaternionX = ahrs_update.quat_x;
            AHRS100.this.quaternionY = ahrs_update.quat_y;
            AHRS100.this.quaternionZ = ahrs_update.quat_z;
        ...
        public void setRawData(AHRSProtocol.GyroUpdate raw_data_update, long sensor_timestamp) {
            AHRS100.this.raw_gyro_x = raw_data_update.gyro_x;
            AHRS100.this.raw_gyro_y = raw_data_update.gyro_y;
            AHRS100.this.raw_gyro_z = raw_data_update.gyro_z;
            AHRS100.this.raw_accel_x = raw_data_update.accel_x;
            AHRS100.this.raw_accel_y = raw_data_update.accel_y;
            AHRS100.this.raw_accel_z = raw_data_update.accel_z;
            AHRS100.this.cal_mag_x = raw_data_update.mag_x;
            AHRS100.this.cal_mag_y = raw_data_update.mag_y;
            AHRS100.this.cal_mag_z = raw_data_update.mag_z;
            AHRS100.this.mpu_temp_c = raw_data_update.temp_c;
        ...
        public void setAHRSData(AHRSProtocol.AHRSUpdate ahrs_update, long sensor_timestamp) {
            ...
            // Magnetometer Data
            AHRS100.this.cal_mag_x = ahrs_update.cal_mag_x;
            AHRS100.this.cal_mag_y = ahrs_update.cal_mag_y;
            AHRS100.this.cal_mag_z = ahrs_update.cal_mag_z;
            ...
            updateDisplacement(AHRS100.this.world_linear_accel_x,
                    AHRS100.this.world_linear_accel_y,
                    update_rate_hz,
                    AHRS100.this.is_moving);
        ...
        void updateBoardStatus(byte op_status, short sensor_status, byte cal_status, byte selftest_status) {
            ...
            if (AHRS100.this.op_status == AHRSProtocol.NAVX_OP_STATUS_NORMAL) {
            ...
            if (((AHRS100.this.cal_status
                ...
                /*
                 * Carefully, only reset the software yaw reset offset if calibration completion
                 * upon
                 */
                /* initial poweron or after board reset occurs. */
                if (poweron_init_completed || AHRS100.this.disconnect_startupcalibration_recovery_pending) {
                    AHRS100.this.disconnect_startupcalibration_recovery_pending = false;
                    AHRS100.this.yaw_angle_tracker.init();
                    Tracer.Trace("navX-Sensor Yaw angle auto-reset to 0.0 due to startup calibration.\n");
                }
            }

            AHRS100.this.op_status = op_status;
            AHRS100.this.sensor_status = sensor_status;
            AHRS100.this.cal_status = cal_status;
            AHRS100.this.selftest_status = selftest_status;
        }

        @Override
        public void yawResetComplete() {
            AHRS100.this.yaw_angle_tracker.reset();
            if (AHRS100.this.enable_boardlevel_yawreset) {
        ...
        public void disconnectDetected() {
            ...
            AHRS100.this.op_status = AHRSProtocol.NAVX_OP_STATUS_ERROR;
            AHRS100.this.sensor_status = 0; /* Clear all sensor status flags */
            /*
             * Flag the need to watch for a startup calibration completion event upon later
             * reconnect.
             */
            AHRS100.this.disconnect_startupcalibration_recovery_pending = true;
