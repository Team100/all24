...
public class DriveTowardNote {
    ...
    public Twist2d apply(SwerveState state, Twist2d input) {
        // clip the input to the unit circleWait
        Pose2d PoseTwist = new Pose2d();
        if (m_noteCamera.getObjectTransform2d() != null) {
            PoseTwist = state.pose().transformBy(m_noteCamera.getObjectTransform2d());
        }
        ...
        Rotation2d bearing = bearing(currentTranslation, PoseTwist.getTranslation());
        ...
        double targetMotion = targetMotion(state, PoseTwist.getTranslation());
