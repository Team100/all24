# pylint: disable=invalid-name,missing-class-docstring,no-member,missing-function-docstring,missing-module-docstring,too-few-public-methods,global-statement
...
import numpy as np
import visgeom as vg  # type:ignore
from shapely import geometry as sg, ops as so  # type:ignore
...
def plot_result(poses, landmarks):
    ...
    for dist, color in zip(
        ...
        #     ax, dist, fill_alpha=0.5, fill_color=color
        # )
...
def plot_ellipse(
    ax, dist, n=50, chi2_val=9.21, fill_alpha=0.0, fill_color="lightsteelblue"
):
    u, s, _ = np.linalg.svd(dist.covariance)
    scale = np.sqrt(chi2_val * s)
