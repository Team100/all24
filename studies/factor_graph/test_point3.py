...
def norm_proxy(point: np.ndarray) -> float:
...
def dot(p: Point3, q: Point3, H: Optional[list[np.array]] = None) -> float:
...
def cross(p: Point3, q: Point3, H: Optional[list[np.array]] = None) -> Point3:
...
def normalize(p: Point3, H: Optional[list[np.array]] = None) -> Point3:
...
def norm3(p: np.ndarray, H: Optional[list[np.array]] = None) -> float:
...
def distance3(
    p1: np.ndarray, q: np.ndarray, H: Optional[list[np.array]] = None
) -> float:
    """OptionalJacobian<1, 3> H1, H2"""
    d: float = np.linalg.norm(q - p1)  # type:ignore
    if H is not None:
...
class TestPoint3(unittest.TestCase):
    def test_dot(self) -> None:
        ...
        assert_allclose(numericalDerivative21(f, p, q), H[0], 1e-9)
        assert_allclose(numericalDerivative22(f, p, q), H[1], 1e-9)

        d = dot(p, r, H)
        self.assertAlmostEqual(1.93, d)
        assert_allclose(numericalDerivative21(f, p, r), H[0], 1e-9)
        assert_allclose(numericalDerivative22(f, p, r), H[1], 1e-9)

        d = dot(p, t, H)
        self.assertAlmostEqual(1.07, d)
        assert_allclose(numericalDerivative21(f, p, t), H[0], 1e-9)
        assert_allclose(numericalDerivative22(f, p, t), H[1], 1e-9)
    ...
    def test_cross(self) -> None:
        ...
        cross(omega, theta, H)
        assert_allclose(numericalDerivative21(f, omega, theta), H[0])
        assert_allclose(numericalDerivative22(f, omega, theta), H[1])
    ...
    def test_cross2(self) -> None:
        ...
        cross(p, q, H)
        assert_allclose(numericalDerivative21(f, p, q), H[0])
        assert_allclose(numericalDerivative22(f, p, q), H[1])

        cross(p, r, H)
        assert_allclose(numericalDerivative21(f, p, r), H[0])
        assert_allclose(numericalDerivative22(f, p, r), H[1])
    ...
    def test_normalize(self) -> None:
        ...
        expectedH = numericalDerivative11(fn, point)
    ...
    def test_norm(self) -> None:
        ...
        expectedH = numericalDerivative11(norm_proxy, point)
        assert_allclose(expectedH, actualH[0])

    def test_distance(self) -> None:
        P = Point3(1.0, 12.8, -32.0)
        Q = Point3(52.7, 4.9, -13.3)
        H = [np.zeros((3, 3)), np.zeros((3, 3))]
        d: float = distance3(P, Q, H)
        expectedDistance: float = 55.5426863
        numH1 = numericalDerivative21(testFunc, P, Q)
        numH2 = numericalDerivative22(testFunc, P, Q)
