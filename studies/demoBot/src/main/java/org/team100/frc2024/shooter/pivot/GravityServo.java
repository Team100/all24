...
public class GravityServo implements Glassy {
    ...
    public void setPosition(double goalRad) {
        setPosition(goalRad,0);
    }


    public void setPosition(double goalRad_S, double goalRad_S2) {
        OptionalDouble opt = getPositionRad();
        if (opt.isEmpty()) {
            Util.warn("GravityServo: Broken sensor!");
            return;
        ...
        State100 m_goal = new State100(goalRad_S, goalRad_S2);
        ...
        final double staticFF = getStaticFF(mechanismPositionRad, goalRad_S, u_FB, u_FF);
    ...
    public void setVelocity(double goalVelocityPercent) {
        OptionalDouble opt = getPositionRad();
        if (opt.isEmpty()) {
            Util.warn("GravityServo: Broken sensor!");
            return;
        ...
        final double gravityTorque = 0.006 * Math.cos(mechanismPositionRad);
        final double u_TOTAL = gravityTorque + goalVelocityPercent;
        m_motor.setDutyCycle(u_TOTAL);
    }
